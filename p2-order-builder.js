(() => {
  ...
  function lineTemplate(type,index,product='Chile Ancho'){
    ...
    return `<article class="order-line" data-order-line>
      <div class="order-line-grid">
        <div class="order-cell product-cell"><label class="order-cell-label">Product</label><select data-line-product aria-label="Product">${productOptions(product)}</select></div>
        <div class="order-cell"><label class="order-cell-label">Qty</label><input data-line-qty aria-label="Quantity" type="number" min="0" step="1" value="24"></div>
        <div class="order-cell"><label class="order-cell-label">Unit</label><select data-line-unit aria-label="Unit"><option ${d.unit==='CASE'?'selected':''}>CASE</option><option ${d.unit==='BAG'?'selected':''}>BAG</option><option ${d.unit==='BOX'?'selected':''}>BOX</option><option ${d.unit==='PALLET'?'selected':''}>PALLET</option><option ${d.unit==='LB'?'selected':''}>LB</option></select></div>
        <div class="order-cell" data-per-unit-field><label class="order-cell-label">LB / Unit</label><input data-line-per aria-label="Pounds per unit" type="number" min="0" step="0.01" value="${d.per}"></div>
        <div class="order-cell"><label class="order-cell-label">${purchase?'Unit Cost':'Unit Price'}</label><input data-line-price aria-label="${purchase?'Unit cost':'Unit price'}" type="number" min="0" step="0.01" value="${purchase?d.cost:d.price}"></div>
        <div class="order-cell lot-cell"><label class="order-cell-label">${purchase?'Supplier Lot #':'Inventory Lot'}</label>${purchase?'<input data-line-lot aria-label="Supplier lot number" placeholder="Enter lot #" value="MX-A5018">':`<select data-line-lot aria-label="Inventory lot">${salesLotOptions(product)}</select>`}</div>
        <div class="order-line-amount"><span class="order-cell-label">Amount</span><strong data-line-total>$0.00</strong></div>
        <button type="button" class="line-remove" data-remove-line aria-label="Remove product line">×</button>
      </div>
      <div class="order-line-meta"><span class="line-number">Item ${index+1}</span><span data-line-equation></span></div>
    </article>`;
  }
  ...
  function orderMarkup(type){
    ...
    return `<div class="order-builder" data-smart-order="${type}">
      <section class="order-info-section">
        <div class="order-section-title"><div><span class="order-kicker">ORDER INFORMATION</span><h3>${purchase?'Vendor & receiving details':'Customer & fulfillment details'}</h3></div></div>
        <div class="order-header-grid">
          ...
          <div class="field invoice-field"><label>${purchase?'Vendor Invoice / Bill #':'Invoice #'}</label><input class="invoice-input" data-invoice-number placeholder="${purchase?'BILL-22715':'INV-22699'}"><div class="invoice-state" data-invoice-state>Enter a unique number.</div></div>
        </div>
      </section>

      <section class="order-items-section">
        <div class="order-section-head">
          <div><span class="order-kicker">ORDER ITEMS</span><h3>Products</h3><p>${purchase?'Enter the supplier lot and package weight for every purchased item.':'Choose the inventory lot and confirm the unit weight for every sold item.'}</p></div>
          <button type="button" class="secondary-btn order-add-btn" data-add-line>+ Add Product</button>
        </div>
        <div class="order-grid-head" aria-hidden="true">
          <span>Product</span><span>Qty</span><span>Unit</span><span>LB / Unit</span><span>${purchase?'Unit Cost':'Unit Price'}</span><span>${purchase?'Supplier Lot #':'Inventory Lot'}</span><span>Amount</span><span></span>
        </div>
        <div class="order-lines" data-order-lines>${lineTemplate(type,0)}</div>
      </section>

      <section class="order-summary-section">
        <div class="order-foot-summary">
          <div><span>Total Weight</span><strong data-order-weight>0 LB</strong></div>
          <div><span>Subtotal</span><strong data-order-subtotal>$0.00</strong></div>
          <div><span>Tax</span><strong>$0.00</strong></div>
          <div class="grand"><span>Order Total</span><strong data-order-total>$0.00</strong></div>
        </div>
      </section>

      <div class="drawer-actions order-actions">
        <button type="button" class="secondary-btn" data-save-smart>Save Draft</button>
        <button type="button" class="primary-btn" data-create-smart disabled>${purchase?'Create Purchase Order':'Create Sales Order'}</button>
      </div>
    </div>`;
  }
  ...
  function attachLine(line,type){
    ...
    const syncUnit=()=>{const isLb=unit.value==='LB'; perField.hidden=isLb; line.classList.toggle('unit-is-lb',isLb); if(isLb) per.value=1; updateTotals(line.closest('[data-smart-order]'))};
