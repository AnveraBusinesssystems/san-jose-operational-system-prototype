...
assert.match(inventory, /type="search" list="add-product-names"/);
assert.match(inventory, /placeholder="Type product name"/);
assert.match(inventory, /function productByName\(name\)/);
assert.match(inventory, /currentLotsForProduct\(productId,preferredLocationId=''/);
assert.match(inventory, /<option value="\$\{esc\(l\.lot_id\)\}">\$\{esc\(l\.supplier_lot_number\)\}<\/option>/);
assert.doesNotMatch(inventory, /<option value="\$\{esc\(p\.product_id\)\}">\$\{esc\(p\.product_name\)\} ·/);
assert.equal(fs.existsSync(path.join(root, 'p2-live-inventory-fixes.js')), false);
