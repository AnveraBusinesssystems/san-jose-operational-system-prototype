...
function canOpenPage(page){return window.SanJoseSystem?.canAccessPage?.(page)!==false}
function defaultAllowedPage(){return ['overview','inventory','receiving','shipping','packing','scanner'].find(canOpenPage)||'inventory'}
function renderNav(){const root=document.getElementById('sidebarNav');if(!root)return;root.innerHTML=navSections.map(([label,items])=>[label,items.filter(([id])=>canOpenPage(id))]).filter(([,items])=>items.length).map(([label,items])=>`<section class="nav-section"><div class="nav-label">${label}</div>${items.map(([id,icon,name])=>`<button class="nav-item ${state.page===id?'active':''}" data-nav="${id}" type="button"><span class="nav-icon">${icon}</span><span>${name}</span></button>`).join('')}</section>`).join('');root.querySelectorAll('[data-nav]').forEach(b=>b.onclick=()=>navigate(b.dataset.nav))}
function navigate(page){const safePage=canOpenPage(page)?page:defaultAllowedPage();state.page=safePage;state.filters={};location.hash=safePage;window.scrollTo({top:0,left:0,behavior:'auto'});renderNav();renderPage()}
...
function renderPage(){if(!canOpenPage(state.page)){navigate(defaultAllowedPage());return}const view=document.getElementById('pageView');const renders={overview:renderOverview,purchase:()=>renderOrders('purchase'),sales:()=>renderOrders('sales'),receiving:renderReceiving,shipping:renderShipping,inventory:renderInventory,packing:renderPacking,replenishment:renderReplenishment,products:renderProducts,parties:renderParties,analytics:renderAnalytics,scanner:renderScanner,admin:renderAdmin};view.innerHTML=(renders[state.page]||renders[defaultAllowedPage()]||renderInventory)();bindPageInteractions()}
...
window.addEventListener('hashchange',()=>{const page=location.hash.replace('#','');if(page&&page!==state.page)navigate(page)});
