...
function renderNav(){const root=document.getElementById('sidebarNav');if(!root)return;root.innerHTML=navSections.map(([label,items])=>`<section class="nav-section"><div class="nav-label">${label}</div>${items.map(([id,icon,name])=>`<button class="nav-item ${state.page===id?'active':''}" data-nav="${id}" type="button"><span class="nav-icon">${icon}</span><span>${name}</span></button>`).join('')}</section>`).join('');root.querySelectorAll('[data-nav]').forEach(b=>b.onclick=()=>navigate(b.dataset.nav))}
function navigate(page){state.page=page;state.filters={};location.hash=page;window.scrollTo({top:0,left:0,behavior:'auto'});renderNav();renderPage()}
